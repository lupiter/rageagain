...
export const militaryTime = (time: string) => {
  ...
  if (/\d+:\d+:\d+/.test(time)) return time;
  ...
  if (hours === 12) {
    if (period === "am") date.setHours(hours - 12, minutes);
    else if (period === "pm") date.setHours(hours, minutes);
  } else {
    if (period === "am") date.setHours(hours, minutes);
    else if (period === "pm") date.setHours(hours + 12, minutes);
  }

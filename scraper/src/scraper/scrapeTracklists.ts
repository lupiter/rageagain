...
export const scraperV1 = (
  ...
  if (!$ps.length) console.warn("Failed to find any P tags");

  $ps.toArray().forEach((p) => {
    const $p = $(p);
    const timeslot = $p.text().trim();

    if (!/am|pm$/gi.test(timeslot)) return;
...
export const scraperV2 = (
  ...
  const findLabel = (nodes: cheerio.Cheerio<AnyNode>): string | undefined => {
    for (let i = nodes.length - 1; i >= 0; i--) {
      ...
      if (
        nodes[i] &&
        (nodes[i].type === ElementType.Tag ||
          nodes[i].type === ElementType.Text) &&
        /\(.*\)/.test(n.text().trim() ?? "")
      ) {
...
export const scrapeTracklist = async (
  ...
  if (!html) throw new Error("Failed to download playlist at " + tracklistUrl);

  try {
    const result = scrapeTracklistHtml(html);
    console.log(
      `Scraper v${result.scraper} found ${result.tracks.length} tracks at "${tracklistUrl}"`,
    );

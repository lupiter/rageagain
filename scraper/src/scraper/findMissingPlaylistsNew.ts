...
const findMissingPlaylists = async () => {
  ...
  do {
    const json = await downloadPage(
      `https://www.abc.net.au/core-next/api/collection/rage/playlist?collectionId=13642802&offset=${offset}&size=${size}`,
    );
    ...
    const newPlaylists = playlists.map((playlist: PlaylistEntry) => {
      ...
      if (!dateString.match(/202\d/)) {
        ...
        if (
          tracklistUrl ==
          "https://www.abc.net.au/rage/playlist/friday-night-22-december-on-abc-tv/103241478"
        ) {
